...
<head>
	...
	[#if productCategory??]
		[@seo type = "productList"]
			...
			[/#if]
			<title>[#if productCategory.resolveSeoTitle()?has_content]${productCategory.resolveSeoTitle()}[#else]${seo.resolveTitle()}[/#if][#if showPowered] - Powered By SHOPEC[/#if]</title>
		[/@seo]
	[#else]
		<title>${message("shop.product.title")}[#if showPowered] - Powered By SHOPEC[/#if]</title>
	[/#if]
	...
	$().ready(function() {
		...
		var data = {
			...
			storeProductCategoryId: '${storeProductCategory.id}',
			brandId: '${brand.id}',
