...
public class Sku extends BaseEntity<Sku> {
	 ...
	 */
	@Transient
	public net.shopec.entity.Store getStore() {
	 ...
	 */
	@Transient
	public BigDecimal getCommission(net.shopec.entity.Store.Type type) {
		...
		if (type != null && getProduct() != null && getProduct().getProductCategory() != null) {
			...
			if (net.shopec.entity.Store.Type.general.equals(type) && productCategory.getGeneralRate() > 0) {
				commission = getPrice().multiply(new BigDecimal(productCategory.getGeneralRate().toString()));
			} else if (net.shopec.entity.Store.Type.self.equals(type) && productCategory.getSelfRate() > 0) {
